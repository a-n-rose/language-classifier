...
#for logging:
script_purpose = 'MFCC_extraction_LANNA_VAD' #will name logfile 
...
noisegroup = 'additional' # Options: 'matched' 'none' 'random' 'additional' (i.e. already have noise, but add noise to make sure it's evenly distributed)
#if no noise, environment_noise = None; otherwise, put name of wavefile here
environment_noise = 'background_noise_poor_recording.wav' #Options: None or wavefile i.e. 'background_noise.wav'  'background_noise_poor_recording.wav'
        ...
        if 'y' in check_variables.lower():
            ...
            if environment_noise: 
                try:
                    ...
                    logging.info("Background noise file: {}".format(environment_noise))
            ...
            for j in range(len(dir_list)):
                ...
                if len(tgz_list) > 0:
                    for t in range(len(tgz_list)):
                        ...
                        tgz_filename = Path(filename).name
