...
def shape4LSTM(data_features_labels_matrix,context_window_size=None):
    if context_window_size is None:
        context_window_size = 9
    #prepping shape for LSTM.  
    #shape = (num_spectrogram_sets/images, 19, 120)
    ...
    X = features.reshape(num_frame_sets,context_window_size*2+1,features.shape[1])
...
if __name__=="__main__":
    try:
        ...
        X_f_train, y_f_train = shape4LSTM(train_f)
        X_f_val, y_f_val = shape4LSTM(val_f)
        X_f_test, y_f_test = shape4LSTM(test_f)
        
        X_m_train, y_m_train = shape4LSTM(train_m)
        X_m_val, y_m_val = shape4LSTM(val_m)
        X_m_test, y_m_test = shape4LSTM(test_m)
        ...
        lstm = Sequential()
        lstm.add(LSTM(320,return_sequences=True,input_shape=(19,120)))
        lstm.add(LSTM(320,return_sequences=True))
        lstm.add(Flatten())
        ...
        #train model
        lstm.fit(X_f_train, y_f_train, validation_data=(X_f_val,y_f_val),epochs=25)
        
        #predict test data
        pred = lstm.predict(X_f_test)
        pred = pred >0.5
